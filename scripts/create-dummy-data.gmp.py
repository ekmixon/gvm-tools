...
def create_data(gmp, count):
    ...
    print(f'{str(count)} random credentials generated.')

    for _ in range(count):
        name = generate_id()
        gmp.create_port_list(name, port_range='T:1-42')
    print(f'{str(count)} random port lists generated.')

    for _ in range(count):
        name = generate_id()
        res = gmp.create_config('085569ce-73ed-11df-83c3-002264764cea', name)
        config_ids.append(res.xpath('@id')[0])
    print(f'{str(count)} random scan configs generated.')
    ...
    print(f'{str(count)} random targets generated.')
    ...
    print(f'{str(count)} random tasks generated.')
